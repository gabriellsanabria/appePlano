...
const fetchData = async () => {
    try {
      const response = await fetch(`https://api.eplano.com.br/api/investimentos/estrutura/user/${userId}`);
      if (response.ok) {
  ...
  const handleExcluirProdutoServico = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/investimentos/estrutura/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
