...
const TableEstimarDespesasInsumos = ({onTotalInvestimentosInsumosChange}) => {
...
const fetchData = async () => {
    try {
      const response = await fetch(`https://api.eplano.com.br/api/investimentos/insumos/user/${userId}`);
      if (response.ok) {
  ...
  const handleExcluirProdutoServico = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/investimentos/insumos/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
