...
const TableEstimarDespesasEquipe = ({ onTotalCustoEquipeChange, addProduto, sideType}) => {
...
const fetchData = async () => {
    try {
      const response = await fetch(`https://api.eplano.com.br/api/simulador/despesas/equipe/user/${userId}`);
      if (response.ok) {
  ...
  const handleExcluirProdutoServico = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/simulador/despesas/equipe/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
