...
const TableEstimarCaixaLiquido = ({ onTotalCaixaLiquidoChange, addProduto, sideType }) => {
...
const fetchData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/caixa/liquido/user/${userId}`);
      if (response.ok) {
        ...
        setApiData(data);
        if (onTotalCaixaLiquidoChange) onTotalCaixaLiquidoChange(totalInvestimentos);
      } else {
  ...
  const handleExcluirProdutoServico = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/caixa/liquido/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
...
export default TableEstimarCaixaLiquido;
