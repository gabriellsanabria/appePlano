...
const TableEstimarCaixaContasAPagar = ({ onTotalContasPagarChange }) => {
...
const fetchData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/caixa/contas_pagar/user/${userId}`);
      if (response.ok) {
        ...
        setApiData(data);
        if (onTotalContasPagarChange) onTotalContasPagarChange(totalInvestimentos);
      } else {
  ...
  const handleExcluirProdutoServico = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/caixa/contas_pagar/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
...
export default TableEstimarCaixaContasAPagar;
