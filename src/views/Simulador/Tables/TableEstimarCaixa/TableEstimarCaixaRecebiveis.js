...
const TableEstimarCaixaRecebiveis = ({ onTotalCaixaRecebiveisChange, addProduto, sideType  }) => {
...
const fetchData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/caixa/recebiveis/user/${userId}`);
      if (response.ok) {
        let data = await response.json();
        // Ordenando os dados por produto_servico em ordem alfabética
        data.sort((a, b) => a.descricao.localeCompare(b.descricao));
       
        const totalInvestimentos = data.reduce((sum, item) => sum + parseFloat(item.valor) || 0, 0);
        setTotalInvestimentos(totalInvestimentos);
  
        setApiData(data);
        if (onTotalCaixaRecebiveisChange) onTotalCaixaRecebiveisChange(totalInvestimentos);
      } else {
  ...
  const handleExcluirProdutoServico = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/caixa/recebiveis/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
...
export default TableEstimarCaixaRecebiveis;
