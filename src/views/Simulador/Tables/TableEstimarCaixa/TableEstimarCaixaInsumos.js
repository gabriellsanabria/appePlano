...
const TableEstimarInvestimentosEstrutura = ({ onTotalInvestimentosInsumosChange, addProduto, sideType }) => {
...
const fetchData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/caixa/estoque/user/${userId}`);
      if (response.ok) {
        ...
        setApiData(data);
        if (onTotalInvestimentosInsumosChange) onTotalInvestimentosInsumosChange(totalInvestimentos);
      } else {
  ...
  const handleExcluirProdutoServico = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/caixa/estoque/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
